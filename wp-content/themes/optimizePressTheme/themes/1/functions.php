...
function theme1_localize_script($array){
    $array['search_default'] = __('Search...', 'optimizepress');
    return $array;
}
add_filter(OP_SN.'-script-localize-front','theme1_localize_script');
...
function theme1_posts_list($sql,$options=array()){
    ...
    if(count($posts) > 0){
        foreach($posts as $post){
            ...
            $link_start = '<a href="'.$permalink.'" title="'.sprintf( esc_attr__( 'Permalink to %s', 'optimizepress'), esc_attr($title) ).'" rel="bookmark">';
            ...
            $commentsHtml = '';
            $commentModule = new OptimizePress_Blog_Comments_Module();

            if($commentModule->get_option('comments','wordpress','enabled') == 'Y')
                $commentsHtml = '<p><a href="'.get_comments_link($post->ID).'">'.sprintf(_n('1 Comment','%1$s Comments',$cn, 'optimizepress'), number_format_i18n( $cn )).'</a></p>';

            $html .= '
    <li'.$class.'>'.$img.'<h4>'.$link_start.theme1_truncate_title(get_the_title($post->ID)).'</a></h4>'.$commentsHtml.'</li>';
...
function theme1_column_widths($widths=array()){
    ...
    if(!isset($newwidths)){
        ...
        if(defined('OP_SIDEBAR') && OP_SIDEBAR === false){
            ...
            $newwidths['main-content'] = ((975-$newwidths['main-sidebar'])-66);
